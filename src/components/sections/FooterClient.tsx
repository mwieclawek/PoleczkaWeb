...
export default function FooterClient({
  ...
  const [data, setData] = useState<Required<SiteSettingsData>>({
    ...
    copyrightText:
      initialData?.copyrightText || defaultSiteSettings.copyrightText,
  });
      ...
      .then((fetched) => {
        if (isMounted && fetched) {
          setData({
            ...
            facebookUrl:
              fetched.facebookUrl || defaultSiteSettings.facebookUrl,
            copyrightText:
              fetched.copyrightText || defaultSiteSettings.copyrightText,
          });
        }
      })
      .catch(() => {});
    ...
    <footer className="bg-[#960C3F] text-[#FFFDF6]">
      <div className="mx-auto max-w-7xl px-6 py-16 lg:px-8 lg:py-20">
        ...
        <div className="flex flex-col items-center justify-between gap-4 text-xs text-[#FFFDF6]/40 md:flex-row">
          ...
          <span>Zbudowane z pasją we Wrocławiu</span>
        </div>
      </div>
    </footer>
  );
